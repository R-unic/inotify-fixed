module Inotify
  ...
  class Watcher
    ...
    def initialize(@recursive : Bool = false)
      ...
      Exception.new "inotify init failed" if fd == -1
    ...
    private def lurk # ameba:disable Metrics/CyclomaticComplexity
      ...
      while @enabled
        ...
        Exception.new "inotify read() failed" if bytes_read == -1
    ...
    def watch(path : String, mask = DEFAULT_WATCH_FLAG)
      ...
      Exception.new "inotify add_watch failed" if wd == -1
    ...
    def unwatch(wd : LibC::Int)
      ...
      if status == -1
        ...
        when Errno::EINVAL then Exception.new "The watch descriptor wd is not valid; or fd is not an inotify file descriptor"
        else
          Exception.new "inotify rm_watch failed"
        end
      end
